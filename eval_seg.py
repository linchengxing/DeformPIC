...
def eval_cd(args, config, base_model, eval_task):
    ...
    test_dataloader = torch.utils.data.DataLoader(test_dataset, batch_size=config.total_bs,
                                                  shuffle=False,
                                                  drop_last=False,
                                                  num_workers=int(args.num_workers),
                                                  worker_init_fn=worker_init_fn)
    shape_avg_iou = eval(base_model, test_dataloader, args, config)
    return shape_avg_iou

def eval(base_model, test_dataloader, args, config):
    color_map_final_path = os.path.join(args.data_path, 'color_map_final.npy')
    color_points_raw = np.load(color_map_final_path)
    ...
    with torch.no_grad():
        for idx, (pointset1_pc, pointset2_pc, target1, target2) in enumerate(test_dataloader):
            ...
            results = base_model.sample(pointset1_pc, pointset2_pc, target1, target2)
            rebuild_points = results['prediction']
            gt_points = results['target']
