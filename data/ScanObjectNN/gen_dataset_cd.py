...
def main():
    ...
    root_path = "data/ScanObjectNN"
    test_output_dir = "data/ScanObjectNN/Test_dataset"
    json_output_dir = "data/ScanObjectNN"

    new_test_list = []

    test_data_path = os.path.join(root_path, "main_split", f"test_objectdataset_augmentedrot_scale75.h5")
    h5 = h5py.File(os.path.join(test_data_path), 'r')
    ...
    for task in tasks:
        level_dict = {}
        combined_list = list(zip(points, labels))
        random.shuffle(combined_list)
        points, labels = zip(*combined_list)
        for idx, point_set in tqdm.tqdm(enumerate(points), total=len(points)):
            ...
            file_name = "category{}_{}.npy".format(labels[idx], idx)
        ...
        print("Total: {} samples".format(len(points)))
