...
def get_fscore(dist1, dist2, threshold=0.001):
    """
...
class PairDataset_MN40_Scan(Dataset):
    def __init__(self, args, eval_task, corrupt_level):
        ...
        self.test_list_file = os.path.join(self.data_root, 'test_list.json')
        ...
        test_list = json.load(open(self.test_list_file))
        self.file_list = []
        self.test_list = []
        ...
        for line in test_list:
            task = line.split('/')[1]
            if task == eval_task:
                self.test_list.append({'task': task, 'file_path': line})
        self.task_dict = {}
        for idx, file in enumerate(self.test_list):
            task = file['task']
            self.task_dict.setdefault(task, []).append(idx)
    ...
    def __getitem__(self, idx):
        ...
        pointset2 = self.test_list[pointset2_index]

        if task == "registration":
            pointset2_pc = np.load(os.path.join(self.data_root, pointset2['file_path']))['pointcloud'].astype(np.float32)
        else:
...
class PairDataset_ShapeNet(Dataset):
    def __init__(self, args, eval_task, corrupt_level):
        self.data_root = args.data_path
        self.corrupt_level = corrupt_level - 1
...
def get_args():
    ...
    parser.add_argument('--total_bs', type=int, default=32, help='total batch size for training')
    # seed
    ...
    parser.add_argument('--dataset_name', type=str, default='ShapeNet', choices=['ShapeNet', 'ScanObjectNN', 'ModelNet40'],
                        help='dataset name, choose from [ShapeNet, ScanObjectNN, ModelNet40]')
    # comment
...
def eval_cd(args, base_model, eval_task, corrupt_level):
    eval_dataset = args.dataset_name
    if eval_dataset == "ShapeNet":
        test_dataset = PairDataset_ShapeNet(args, eval_task, corrupt_level)
    elif eval_dataset == "ScanObjectNN":
        test_dataset = PairDataset_MN40_Scan(args, eval_task, corrupt_level)
    elif eval_dataset == "ModelNet40":
        test_dataset = PairDataset_MN40_Scan(args, eval_task, corrupt_level)
    else:
        raise NotImplementedError(f"Dataset {eval_dataset} is not supported.")
        
    test_dataloader = torch.utils.data.DataLoader(test_dataset, batch_size=args.total_bs,
                                                  shuffle=False,
                                                  drop_last=False,
...
def eval(base_model, test_dataloader):
    ...
    with torch.no_grad():
        for idx, (pointset1_pc, pointset2_pc, target1, target2) in enumerate(test_dataloader):
            ...
            rebuild_points = base_model.sample(pointset1_pc, pointset2_pc, target1, target2)['prediction']
            ...
            dist1, dist2 = ChamferFunction.apply(rebuild_points, target2)
            fscore = get_fscore(
                dist1, dist2,
                threshold=0.001
            )
            mean_fscore += fscore[0].mean().item()*100
            mean_emd += emd()(rebuild_points, target2).mean().item()*1000
            i += 1
        mean_loss /= i
        mean_fscore /= i
        mean_emd /= i
    return mean_loss, mean_fscore, mean_emd
...
def main():
    ...
    for eval_task in tasks:
        ...
        for corrupt_level in range(5):
            loss, fscore, emd = eval_cd(args, base_model, eval_task, corrupt_level+1)
            print_log('[TEST] corrupt_level: {} loss = {:.4f}, fscore = {:.4f} %, emd = {:.4f}'.format(corrupt_level+1, loss, fscore, emd), logger=logger)
            task_loss += loss
            task_fscore += fscore
            task_emd += emd
        task_loss /= 5
        task_fscore /= 5
        task_emd /= 5
        print_log('[TEST {}] loss = {:.4f}, fscore = {:.4f} %, emd = {:.4f}'.format(eval_task.upper(), task_loss, task_fscore, task_emd), logger=logger)
    print_log("Done!!!!!!!!!!!!!!!!!")
