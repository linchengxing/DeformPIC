...
def build_opti_sche(base_model, config):
    ...
    elif sche_config.type == 'Constant':
        scheduler = None
    else:
        raise NotImplementedError()
...
def resume_logger(args, logger = None):
    ckpt_path = os.path.join(args.experiment_path, 'ckpt-last.pth')
    if not os.path.exists(ckpt_path):
        print_log(f'[RESUME INFO] no checkpoint file from path {ckpt_path}...', logger = logger)
        return None
    print_log(f'[RESUME INFO] Loading logger run id from {ckpt_path}...', logger = logger )
    # load state dict
    state_dict = torch.load(ckpt_path, map_location='cpu')
    return state_dict.get('logger_run_id', None)

def save_checkpoint(base_model, optimizer, epoch, metrics, best_metrics, prefix, args, swanlab_run, logger = None):
    if args.local_rank == 0:
        torch.save({
                    ...
                    'logger_run_id': swanlab_run.id if (swanlab_available and swanlab_run is not None) else None
