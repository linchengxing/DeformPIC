...
class PairDataset(data.Dataset):
    def __init__(self, config):
        ...
        self.valid_tasks = config.valid_tasks
        ...
        if self.subset == "train":
            ...
            for idx, file in enumerate(self.file_list):
                ...
                if task == "partsegmentation":
                    ...
                    category_dict = self.task_dict[task]['category_dict']
                    category_dict.setdefault(category, []).append(idx)

                elif task == "non_rigid_registration":
                    file_name = file['file_path'].split('/')[-1]
                    category = file_name.split('-')[0]
                    level = file['file_path'].split('/')[-2]
                    if task not in self.task_dict:
                        self.task_dict[task] = {
                            'category_dict': {},
                            'level_dict': {}
                    ...
                    category_dict = self.task_dict[task]['category_dict']
                    level_dict = self.task_dict[task]['level_dict']
                    category_dict.setdefault(category, []).append(idx)
                    level_dict.setdefault(level, []).append(idx)
                else:
    ...
    def __getitem__(self, idx):
        ...
        if task == "partsegmentation":
            # sample category from the same category
            file_name = pointset1['file_path'].split('/')[-1]
            category = file_name.split('-')[0]
            category_dict = self.task_dict[task]['category_dict']
            pointset2_index = random.choice(category_dict[category])
        elif task == "non_rigid_registration":
            # sample category from the same category and the same level
            file_name = pointset1['file_path'].split('/')[-1]
            category = file_name.split('-')[0]
            level = pointset1['file_path'].split('/')[-2]
            category_dict, level_dict = self.task_dict[task]['category_dict'], self.task_dict[task]['level_dict']
            # Intersection of category_dict[category] and level_dict[level]
            pointset2_index = random.choice(list(set(category_dict[category]) & set(level_dict[level])))
        else:
